...
class AppTest < Minitest::Test
  ...
  def test_renders_discord_like_scrolling_shell
    Dir.mktmpdir do |dir|
      ...
      assert_includes response.body, "send-control"
      assert_includes response.body, "send-button"
      assert_includes response.body, "data-send-menu-toggle"
      assert_includes response.body, "aria-controls=\"send-options\""
      assert_includes response.body, "id=\"send-options\" data-send-menu hidden"
      assert_includes response.body, "data-streaming-behavior=\"steer\" aria-pressed=\"true\""
      assert_includes response.body, "data-streaming-behavior=\"follow_up\" aria-pressed=\"false\""
      assert_includes response.body, ">Queue follow-up</button>"
      refute_includes response.body, "<fieldset class=\"streaming-behavior\""
      refute_includes response.body, "name=\"streaming_behavior\""
      ...
      assert_includes APP_STYLESHEET, ".message--tool .message-details-summary, .message--tool-call .message-details-summary, .message--tool-transcript .message-details-summary { max-width: 100%; overflow-x: auto; white-space: nowrap; font-family: var(--mono); font-size: 0.84rem; }"
  ...
  def test_session_switching_script_replaces_session_regions
    Dir.mktmpdir do |dir|
      ...
      assert_includes APP_JAVASCRIPT, "function switchSession(url, { push = true, focus = true, preserveScroll = false, findQuery = null } = {})"
  ...
  def test_session_switching_script_resets_polling_and_attachments
    Dir.mktmpdir do |dir|
      ...
      assert_includes APP_JAVASCRIPT, "const submittedPromptSuperseded = () => submissionGeneration !== promptSubmissionGeneration;"
      assert_includes APP_JAVASCRIPT, "if (stopHandlingChangedSubmittedView()) return;"
      assert_operator APP_JAVASCRIPT.scan("if (submittedPromptSuperseded()) return;").length, :>=, 4
  ...
  def test_live_event_script_keeps_assistant_and_status_roles_separate
    Dir.mktmpdir do |dir|
      ...
      assert_includes APP_JAVASCRIPT, "const agentBusy = [\"running\", \"sending\", \"stopping\"].includes(state);"
      ...
      assert_includes APP_JAVASCRIPT, "if ([\"running\", \"sending\"].includes(currentState)) selectStreamingBehavior(streamingBehavior, { focus: false });"
      assert_includes APP_JAVASCRIPT, "if (currentState === \"sending\") setComposerState(\"running\""
      assert_includes APP_JAVASCRIPT, "if (![\"running\", \"sending\"].includes(state)) streamingBehaviorSelection = \"steer\";"
      assert_includes APP_JAVASCRIPT, "keyboardStreamingBehaviorOverride = event.altKey ? \"follow_up\" : null"
      assert_includes APP_JAVASCRIPT, "event.key === \"Enter\" && !event.shiftKey"
      assert_includes APP_JAVASCRIPT, "sendControl.classList.toggle(\"is-streaming\", running);"
      assert_includes APP_JAVASCRIPT, "sendMenuToggle.hidden = !running || forcedFollowUp;"
      assert_includes APP_JAVASCRIPT, "const forcedFollowUp = liveOutput?.dataset.composerCompacting === \"true\";"
      assert_includes APP_JAVASCRIPT, "selectStreamingBehavior(button.dataset.streamingBehavior)"
