...
Gripi preserves malformed gateway-owned JSON state instead of silently replacing it. Requests which depend on that state return an internal error, and the gateway log identifies the affected state. To recover, stop the gateway, inspect and back up the configured file, then repair it or move it aside before restarting. Moving a file resets only its corresponding gateway metadata, such as browser/workspace approvals, pinned sessions, or read markers; it does not modify Pi session JSONL files.
